...
def index():
    """Ana sayfa"""
    logger.info("Ana sayfa talep edildi")
    return render_template('index.html')

@app.route('/query/<query_name>')
def query_page(query_name):
    """Sorgu sayfasını render eder"""
    logger.info(f"Sorgu sayfası talep edildi: {query_name}")
...
def execute_query():
    ...
    try:
        ...
        logger.info(f"Sorgu çalıştırılıyor: {query_key}, Parametreler: {params}")
        ...
        # API'yi çağır
        logger.info(f"API çağrısı: {api_url}")
        ...
        return jsonify({
            "success": False,
            "error": "API zaman aşımı (30 saniye)"
        }), 504
...
def get_query_info(query_key):
    ...
    logger.info(f"Sorgu bilgisi talep edildi: {query_key}")
